import Posts from "../models/postModel.js";
import Views from "../models/viewsModel.js";
import User from "../models/userModel.js";
import mongoose from "mongoose";
import Followers from "../models/followersModel.js";
import Comments from "../models/commentModel.js";
//admin endpoints
export const stats = async (req, res, next) => {
  try {
    const { query } = req.query;
    const { userId } = req.user.user;
    const numDays = Number(query) || 30;
    const currentDate = new Date();
    const startDate = new Date();
    startDate.setDate(currentDate.getDate() - numDays);

    //Total stats
    const totalPost = await Posts.find({
      user: userId,
      createdAt: { $gte: startDate, $lte: currentDate },
    }).countDocuments();
    const totalView = await Views.find({
      user: userId,
      createdAt: { $gte: startDate, $lte: currentDate },
    }).countDocuments();
    const totalWritters = await User.find({
      accountType: "Writer",
    }).countDocuments();
    const totalFollowers = User.findById(userId);
    const viewStats = Views.aggregate([
      {
        $match: {
          user: new mongoose.Types.ObjectId(userId),
          createdAt: { $gte: startDate, $lte: currentDate },
        },
      },
      {
        $group: {
          _id: {
            $dateToString: { format: "%Y-%m-%d", date: "$cratedAt" },
          },
          Total: { $sum: 1 },
        },
      },
      { $sort: { _id: 1 } },
    ]);
    const followerStats = Followers.aggregate([
      {
        $match: {
          user: new mongoose.Types.ObjectId(userId),
          createdAt: { $gte: startDate, $lte: currentDate },
        },
      },
      {
        $group: {
          _id: {
            $dateToString: { format: "%Y-%m-%d", date: "$cratedAt" },
          },
          Total: { $sum: 1 },
        },
      },
      { $sort: { _id: 1 } },
    ]);
    const lastFollowers = await User.findById(userId).populate({
      path: "followers",
      options: { sort: { _id: -1 } },
      perDocumentLimit: 5,
      populate: {
        path: "followersId",
        select: "name email image accountType followers password",
      },
    });
    const lastPosts = await Posts.findById({ user: userId })
      .limit(5)
      .sort({ _id: -1 });

    res.status(200).json({
      success: true,
      message: "Data loaded successfully",
      totalPost,
      totalView,
      totalWritters,
      viewStats,
      followers: totalFollowers?.followers?.lenght,
      followerStats,
      lastFollowers: lastFollowers?.followers,
      lastPosts,
    });
  } catch (error) {
    console.log(error);
    res.status(404).json({ message: "Something went wrong" });
  }
};
export const getFollowers = async (req, res, next) => {
  try {
    const { userId } = req.body.user;
    //pagination
    const page = Number(req.query.page) || 1;
    const limit = Number(req.query.limit) || 8;
    const skip = (page - 1) * limit;

    const result = await User.findById(userId).populate({
      path: "followers",
      options: { sort: { _id: -1 }, limit: limit, skip: skip },
      populate: {
        path: "followersId",
        select: "name email image accountType followers password",
      },
    });

    const totalFollowers = await User.findById(userId);
    const numPages = Math.ceil(totalFollowers?.followers?.length / limit);
    res.status(200).json({
      data: result?.followers,
      total: totalFollowers?.followers.length,
      numPages,
      page,
    });
  } catch (error) {
    console.log(error);
    res.status(404).json({ message: "Something went wrong" });
  }
};
export const getPostContent = async (req, res, next) => {
  try {
    const { userId } = req.body.user;
    let queryResult = Posts.find({ user: userId }).sort({
      _id: -1,
    });

    //pagination
    const page = Number(req.query.page) || 1;
    const limit = Number(req.query.limit) || 8;
    const skip = (page - 1) * limit;

    //records count
    const totalPost = await Posts.countDocuments({ user: userId });
    const numPage = Math.cell(totalPost / limit);

    queryResult = queryResult.skip(skip).limit(limit);
    const posts = await queryResult;
    res.status(200).json({
      success: true,
      message: "Content Loaded successfully",
      numPage,
      data: posts,
      totalPost,
      page,
    });
  } catch (error) {
    console.log(error);
    res.status(404).json({ message: "Something went wrong" });
  }
};
export const createPost = async (req, res, next) => {
  try {
    const { userId } = req.body.user;
    const { title, slug, desc, img, cat } = req.body;
    if (!(desc || img || title || cat || slug)) {
      return next("All fields are required, Please try again");
    }
    const post = await Posts.create({
      user: userId,
      title,
      desc,
      slug,
      img,
      cat,
    });
    res.status(200).json({
      success: true,
      message: "Post created successfully",
      data: post,
    });
  } catch (error) {
    console.log(error);
    res.status(404).json({ message: "Something went wrong" });
  }
};

//like and comment post endpoints
export const commentPost = async (req, res, next) => {
  try {
    const { desc } = req.body;
    const { userId } = req.body.user;
    const { id } = req.params;

    if (desc === null) {
      res.status(404).json({ message: "Comment is required" });
    }

    const newComment = new Comments({
      desc,
      user: userId,
      post: id,
    });

    await newComment.save();

    //update comment
    const post = await Posts.findById(id);

    post.comments.push(newComment._id);

    await Posts.findByIdAndUpdate(id, post, {
      new: true,
    });

    res.status(200).json({
      success: true,
      message: "Comment published successfully",
      newComment,
    });
  } catch (error) {
    console.log(error);
    res.status(404).json({ message: "Something went wrong" });
  }
};

//update post enpoints
export const updatePost = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { status } = req.body;

    const post = await Posts.findByIdAndUpdate(id, { status }, { new: true });
    res.status(200).json({
      success: true,
      message: "Action peformed successfully",
      data: post,
    });
  } catch (error) {
    console.log(error);
    res.status(404).json({ message: "Something went wrong" });
  }
};

//get post endpoints
export const getPosts = async (req, res, next) => {
  try {
    const { cat, writerId } = req.query;
    let query = { status: true };
    if (cat) {
      query.cat = cat;
    } else if (writerId) {
      query.user = writerId;
    }
    let queryResult = Posts.find(query)
      .populate({
        path: "user",
        select: "name image password",
      })
      .sort({ _id: -1 });
    //pagination
    const page = Number(req.query.page) || 1;
    const limit = Number(req.query.limit) || 8;
    const skip = (page - 1) * limit;

    //records count
    queryResult = queryResult.skip(skip).limit(limit);
    const totalPost = await Posts.countDocuments(queryResult);
    const numPage = Math.ceil(totalPost / limit);

    
    const posts = await queryResult;
    res.status(200).json({
      success: true,
      numPage,
      data: posts,
      totalPost,
      page,
    });
  } catch (error) {
    console.log(error);
    res.status(404).json({ message: "Something went wrong" });
  }
};
export const getPopularPost = async (req, res, next) => {
  try {
    const posts = await Posts.aggregate([
      {
        $match: {
          status: true,
        },
      },
      {
        $project: {
          title: 1,
          slug: 1,
          img: 1,
          cat: 1,
          views: { $size: "$views" },
          createdAt: 1,
        },
      },
      {
        $sort: { views: -1 },
      },
      {
        $limit: 5,
      },
    ]);
    const writters = await User.aggregate([
      {
        $match: {
          accountType: { $ne: "User" },
        },
      },
      {
        $project: {
          name: 1,
          image: 1,
          followers: { $size: "$followers" },
        },
      },
      {
        $sort: { views: -1 },
      },
      {
        $limit: 5,
      },
    ]);
    res.status(200).json({
      success: true,
      message: "Succesfull",
      data: { posts, writters },
    });
  } catch (error) {
    console.log(error);
    res.status(404).json({ message: "Something went wrong" });
  }
};
export const getPost = async (req, res, next) => {
  try {
    const { postId } = req.params;
    const post = await Posts.findById(postId).populate({
      path: "user",
      select: "name image",
    });

    const newView = await Views.create({
      user: post?.user,
      post: postId,
    });
    post.views.push(newView?._id);

    await Posts.findByIdAndUpdate(postId, post);

    res.status(200).json({
      success: true,
      message: "Succesfull",
      data: post,
    });
  } catch (error) {
    console.log(error);
    res.status(404).json({ message: "Something went wrong" });
  }
};
export const getComments = async (req, res, next) => {
  try {
    const { postId } = req.params;
    const postComments = await Comments.find({ post: postId })
      .populate({
        path: "user",
        select: "name image password",
      })
      .sort({ _id: -1 });
    res.status(200).json({
      success: true,
      message: "Succesfull",
      data: postComments,
    });
  } catch (error) {
    console.log(error);
    res.status(404).json({ message: "Something went wrong" });
  }
};

//delete post endpoints
export const deletePost = async (req, res, next) => {
  try {
    const { userId } = req.body.user;
    const { id } = req.params;
    console.log(userId, id);
    await Posts.findByIdAndDelete({ _id: id, user: userId });
    res.status(200).json({
      success: true,
      message: "Delete Succesfully",
    });
  } catch (error) {
    console.log(error);
    res.status(404).json({ message: "Something went wrong" });
  }
};
export const deleteComment = async (req, res, next) => {
  try {
    const { postId,id } = req.params;
    await Comments.findByIdAndDelete(id);
    //remove comment from post
    const result = await Posts.updateOne(
      { _id: postId },
      { $pull: { comments:id} }
    );
    if (result.modifiedCount > 0) {
      res.status(200).json({
        success: true,
        message: "Comment removed successfully",
      });
    } else {
      res.status(404).json({
        success: false,
        message: "Post or comment not found",
      });
    }
  } catch (error) {
    console.log(error);
    res.status(404).json({ message: "Something went wrong" });
  }
};
